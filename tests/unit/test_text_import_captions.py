...
def test_import_text_excludes_captions_by_default(tmp_path, monkeypatch):
...
def test_import_text_includes_valid_captions_when_requested(tmp_path, monkeypatch):
    experiment = _experiment(tmp_path)
    _write_manifests(tmp_path)
    index = FakeTextIndex()
    monkeypatch.setattr("indexing.extract_text.build_text_index", lambda experiment: index)

    assert import_text(experiment, include_captions=True) == 2
...
def test_import_text_supports_a_caption_only_artifact(tmp_path, monkeypatch):
    ...
    assert import_text(experiment, include_captions=True) == 1
    assert set(index.documents) == {"f1__caption"}


def test_import_text_parser_disables_captions_by_default_and_supports_opt_in():
    parser = build_parser()

    default = parser.parse_args(["import-text", "--experiment-name", "result"])
    enabled = parser.parse_args(
        ["import-text", "--experiment-name", "result", "--include-captions"]
    )

    assert default.include_captions is False
    assert enabled.include_captions is True
