...
def _processed(**overrides):
    values = {
        ...
        "ocr_keywords": [],
        "asr_keywords": [],
        "normalized_keywords": {"ocr": [], "asr": []},
        "metadata": {},
        "weights": {"ocr_bonus": 0.0, "asr_bonus": 0.0},
        "routing_mode": "heuristic",
        "modality_confidence": {"kis": 1.0, "ocr": 0.0, "asr": 0.0},
        "llm_status": "disabled",
...
def test_no_text_hits_leave_kis_ranking_unchanged(monkeypatch, tmp_path) -> None:
    ...
    retriever.query_processor.process.return_value = _processed(
        ...
        normalized_keywords={"ocr": ["giam gia"], "asr": []},
        modality_confidence={"kis": 1.0, "ocr": 1.0, "asr": 0.0},
        weights={"ocr_bonus": 0.3, "asr_bonus": 0.0},
    )
    retriever.search_processed.return_value = [
...
def test_text_backend_failure_preserves_kis_results(monkeypatch, tmp_path) -> None:
    ...
    retriever = MagicMock()
    retriever.query_processor.process.return_value = _processed(
        ocr_keywords=["cửa hàng"],
        normalized_keywords={"ocr": ["cua hang"], "asr": []},
        modality_confidence={"kis": 1.0, "ocr": 0.8, "asr": 0.0},
        weights={"ocr_bonus": 0.4, "asr_bonus": 0.0},
    )
    retriever.search_processed.return_value = [SearchResult("f1", "v1", 0.9, shot_id="s1")]
    text_index = MagicMock()
    ...
    assert response["component_status"]["ocr"] == "error"
    assert response["fusion_weights"]["ocr"] == 0.0
    assert "elasticsearch down" in response["component_stats"]["ocr"]["error"]
