...
`visual-only`, `ocr-only`, `asr-only`, `visual-ocr`, `visual-asr`, `ocr-asr`
và `temporal` (tổng cộng 70 query). Mỗi nhóm nên có cả query tiếng Việt có
dấu và không dấu. Báo cáo evaluator gồm số liệu tổng và riêng cho từng giá
trị `group` để so sánh các ablation.
