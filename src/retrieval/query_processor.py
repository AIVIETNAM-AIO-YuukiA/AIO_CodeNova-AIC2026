...
class LlmQueryProcessor(QueryProcessor):
    ...
    def process(self, query: str) -> ProcessedQuery:
        ...
        1. "visual_prompt": If the query is in Vietnamese, translate it to English. Then rewrite it as a visually descriptive search prompt optimized for CLIP-based video retrieval. Preserve the original intent and entities exactly. Add only generic visual cues that can be reasonably inferred from the query (e.g., scene type, viewpoint, lighting, composition, colors, camera shot). Do not invent actions, objects, people, locations, events, or attributes that are not stated or strongly implied by the query. Keep the output concise (1–2 sentences, max 50 words).
