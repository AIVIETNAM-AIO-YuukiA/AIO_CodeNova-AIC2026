"""Adaptive multimodal Intelligent Search over visual, OCR, ASR and captions."""
...
_COMPONENTS = ("kis", "ocr", "asr", "caption")
...
def intelligent_search(
    ...
    if text_search_mode == "joint" and enable_ocr and enable_asr:
        ...
        if enable_ocr and processed.ocr_keywords and confidences["ocr"] > 0:
            text_specs.append(
                ("ocr", _keywords_for(processed, "ocr"), "ocr", confidences["ocr"])
            )
        if enable_asr and processed.asr_keywords and confidences["asr"] > 0:
            text_specs.append(
                ("asr", _keywords_for(processed, "asr"), "asr", confidences["asr"])
            )
    if enable_caption and processed.caption_keywords and confidences["caption"] > 0:
        text_specs.append(
            (
                "caption",
                _keywords_for(processed, "caption"),
                "caption",
                confidences["caption"],
            )
        )
    ...
    for component, keywords, sources, confidence in text_specs:
        ...
        outcome.stats.update(
            {"router_confidence": confidence, "effective_weight": weight}
        )
        ...
        if component == "joint_text":
            ...
            component_status[component] = (
                "used" if outcome.results and weight > 0 else "no_hits"
            )
    ...
    for component in ("ocr", "asr", "caption"):
    ...
    apply_evidence_reranker = (
        use_evidence_reranker
        if use_evidence_reranker is not None
        else use_reranker is not False
    )
    if apply_evidence_reranker:
        try:
            ...
            LOGGER.exception(
                "event=EVIDENCE_RERANKER_DEGRADED; returning pre-rerank SRRF results"
            )
...
def _search_text(
    ...
    nearest = (
        NearestFrameIndex(experiment)
        if "asr" in sources or "ocr" in sources
        else None
    )
    ...
    document_quality = (
        sum(row[2] for row in ranked_rows[:10]) / min(len(ranked_rows), 10)
        if ranked_rows
        else 0.0
    )
    hit_quality = (
        0.5 * keyword_coverage + 0.3 * score_separation + 0.2 * document_quality
    )
    ...
    if normalized is None:
        normalized = {
            component: [
                ...
                for value in getattr(processed, f"{component}_keywords", [])
            ]
            for component in ("caption", "ocr", "asr")
        }
    return {
