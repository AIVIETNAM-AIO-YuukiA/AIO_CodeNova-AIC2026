...
class Blip2ItmReranker(Reranker):
    ...
    def rerank(self, query: str, results: list[SearchResult]) -> list[SearchResult]:
        ...
        valid_results = [r for r in results if r.frame_path and Path(r.frame_path).exists()]
    ...
    def _score_batch(
        ...
        try:
            for result in batch:
                images.append(Image.open(result.frame_path).convert("RGB"))
    ...
    def _load(self):
        ...
        model = Blip2ForImageTextRetrieval.from_pretrained(
            self.model_name,
            # Use fp16 on GPU for faster inference; fp32 on CPU for compatibility.
